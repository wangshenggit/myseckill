...
import com.seckill.dis.cache.facade.vo.SkKeyPrefix;
...
import com.seckill.dis.goods.utils.VerifyCodeUtil;
...
public class SeckillController implements InitializingBean {
    ...
    /**
     * 用于内存标记，标记库存是否为空，从而减少对redis的访问，内存比redis访问速度更快
     */
    public static volatile Map<Long, Boolean> localOverMap = new HashMap<>();
    ...
    @ResponseBody
    public Result<Integer> testDoSeckill(@RequestParam("goodsId") long goodsId, @RequestParam("uuid") Long uuid) {
        ...
        if (over){
            logger.info("秒杀，商品已结束，goodsId={},uuid={}",goodsId, uuid);
            return Result.error(CodeMsg.SECKILL_OVER);
        }

        // 预减库存，同时在库存为0时标记该商品已经结束秒杀
        Long stock = redisService.decr(GoodsKeyPrefix.GOODS_STOCK, "" + goodsId);
        logger.info("秒杀，商品库存，stock={}",stock);
        if (stock < 0) {
            // 秒杀结束。标记该商品已经秒杀结束
            localOverMap.put(goodsId, true);
            //回滚库存，最终一致
            Long stockS = redisService.incr(GoodsKeyPrefix.GOODS_STOCK, "" + goodsId);
            logger.info("秒杀，商品库存小于0，秒杀结束，库存回滚，stock={}",stockS);
            return Result.error(CodeMsg.SECKILL_OVER);
        }
...
//        }
        ...
        try{
            ...
            logger.info("秒杀，秒杀消息放入消息队列，message={}", JSON.toJSONString(message));
        }catch (Exception e){
            e.printStackTrace();
            //异常，回滚库存
            Long stockSS = redisService.incr(GoodsKeyPrefix.GOODS_STOCK, "" + goodsId);
            logger.error("秒杀，秒杀消息放入消息队列异常，库存回滚，stockS={}", stockSS);
            //秒杀失败
            return Result.success(-5);
        }
        logger.info("秒杀，秒杀成功，goodsId={},uuid={}",goodsId, uuid);
        // 排队中
    ...
    public Result<Integer> doSeckill(Model model, UserVo user,
        ...
        Boolean over = localOverMap.get(goodsId);
        if (over){
            logger.info("秒杀，商品已结束，goodsId={},uuid={}",goodsId, user.getUuid());
            return Result.error(CodeMsg.SECKILL_OVER);
        }
...
//        }
        ...
        logger.info("秒杀，秒杀成功，goodsId={},uuid={}",goodsId, user.getUuid());
        // 排队中
        return Result.success(0);
    }
