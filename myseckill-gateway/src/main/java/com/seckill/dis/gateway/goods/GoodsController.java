...
import com.seckill.dis.gateway.seckill.SeckillController;
...
public class GoodsController {
     ...
     * @return
     */
    @RequestMapping(value = "getDetails/{goodsId}", produces = "text/html")
    @ResponseBody
    public String getDetails(HttpServletRequest request,
                             HttpServletResponse response,
                             Model model,
                             UserVo user, @PathVariable("goodsId") long goodsId) {
        logger.info("获取商品详情");
...
//        goodsDetailVo.setSeckillStatus(skStatus);
        ...
        model.addAttribute("seckillStatus", skStatus);
        ...
        if (!StringUtils.isEmpty(html)) // 如果html文件不为空，则将页面缓存在redis中
        {
            redisService.set(GoodsKeyPrefix.goodsDetailKeyPrefix, "", html);
        }
     ...
     * @return
     */
    @RequestMapping(value = "doTakeOff/{goodsId}")
    @ResponseBody
    public Result<Boolean> doTakeOff(HttpServletRequest request,
                             HttpServletResponse response,
                             Model model,
                             UserVo user, @PathVariable("goodsId") long goodsId) {
        logger.info("商品下架");

        SeckillController.localOverMap.put(goodsId, true);

        // 通过商品id在数据库查询
        GoodsVo goods = new GoodsVo();
        goods.setId(goodsId);
        Boolean isTakeOffSuccess = goodsService.doTakeOff(goods);
        return Result.success(isTakeOffSuccess);
    }
     ...
     * @return
     */
    @RequestMapping(value = "doShelves/{goodsId}")
    @ResponseBody
    public Result<Boolean> doShelves(HttpServletRequest request,
                             HttpServletResponse response,
                             Model model,
                             UserVo user, @PathVariable("goodsId") long goodsId) {
        logger.info("商品上架");

        SeckillController.localOverMap.put(goodsId, false);
        // 通过商品id在数据库查询
        GoodsVo goods = new GoodsVo();
        goods.setId(goodsId);
        Boolean isShelvesSuccess = goodsService.doShelves(goods);
        return Result.success(isShelvesSuccess);
    }
}
